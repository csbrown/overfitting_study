    ...
    def _init_graph(self,x=None,y_=None,summaries=True,trainable=True):
        with tf.name_scope("cifar100_scm"):
            ...
            with tf.name_scope("layer_5_fc"):
                ...
                layer_5_out_dim = self.output_dim
                self.W_fc2 = tf.Variable(tf.truncated_normal(layer_5_in_dim + layer_5_out_dim, stddev=0.1), name="weights", trainable=trainable)
                self.b_fc2 = tf.Variable(tf.constant(0.1,shape=layer_5_out_dim), name="bias", trainable=trainable)
