    ...
    def _init_graph(self,x=None,y_=None,summaries=True,trainable=True):
        with tf.name_scope("cifar10_scm"):
            ...
            with tf.name_scope("layer_5_fc"):
                ...
                self.W_fc2 = tf.Variable(tf.truncated_normal(layer_5_in_dim + [10], stddev=0.1), trainable=trainable)
                self.b_fc2 = tf.Variable(tf.constant(0.1,shape=[10]), trainable=trainable)
